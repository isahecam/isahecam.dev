...
const variantClasses: Record<BadgeVariant, { bg: string; icon: string; shine: string }> = {
  default: {
    bg: "bg-foreground",
...
function BadgeTooltip({ children, content }: { children: ReactNode; content?: string }) {
  ...
  return (
    <BadgeTooltip content={tooltip}>
      <span className={cn("group inline-flex items-center gap-1.5", className)}>
        <span className={cn("relative flex items-center justify-center overflow-hidden rounded-full", bg, container)}>
          ...
          {icon ? (
            <span className={cn("relative z-10 flex items-center justify-center", iconSize, iconColor)}>{icon}</span>
          ) : (
            <CheckIcon className={cn("relative z-10", iconSize, iconColor)} />
          )}
        </span>

        {showLabel && <span className={cn("font-medium text-foreground", text)}>{label}</span>}
      </span>
    </BadgeTooltip>
  );
}
...
export function VerifiedBadgeOutline({
  ...
  return (
    <BadgeTooltip content={tooltip}>
      <span className={cn("group relative inline-flex items-center justify-center", container, className)}>
        <svg viewBox="0 0 24 24" className="absolute inset-0 size-full">
          <circle cx="12" cy="12" r="10" fill="none" strokeWidth="2" className={cn(stroke, "transition-opacity")} />
          <circle
            cx="12"
            cy="12"
            r="10"
        ...
        {icon ? (
          <span className={cn("relative z-10 flex items-center justify-center", iconSize, text)}>{icon}</span>
        ) : (
      ...
      <span className={cn("group relative inline-flex items-center justify-center", className)}>
        <svg viewBox="0 0 24 28" className={sizeMap[size]}>
          ...
          <rect
            ...
            width="24"
            height="28"
            className={cn("fill-white/30", "group-hover:translate-x-48", "transition-transform duration-500 ease-out")}
            clipPath={`url(#shield-clip-${size})`}
          />
        </svg>
        {icon ? (
          <span className={cn("absolute z-10 flex items-center justify-center", iconSize, iconColor)}>{icon}</span>
        ) : (
