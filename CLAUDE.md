...
**UI components**: `components.json` configures shadcn with `style: "base-luma"` on Base UI (`@base-ui/react`), not Radix — do not add Radix-based shadcn components. Aliases: `@/components`, `@/components/ui`, `@/lib`, `@/hooks`. Global styles are at `src/app/[locale]/globals.css` (the app itself was moved under `[locale]`; `components.json`'s `tailwind.css` points there too). Theming uses `@wrksz/themes` (`ThemeProvider` in the root layout, class-based, system-aware).
